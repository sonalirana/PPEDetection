#include "deepstream_ppe.h"

namespace PPE_DS {
  void
  DsPPE::update_fps(gint id) {
  ...
  int
  DsPPE::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  DsPPE::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  DsPPE::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  DsPPE::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
  ...
  gboolean
  DsPPE::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  DsPPE::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  DsPPE::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  DsPPE::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  DsPPE::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  DsPPE::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  DsPPE::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        PPE_DS::DsPPE::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", PPE_DS::DsPPE::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!PPE_DS::DsPPE::set_tracker_properties(nvtracker)) {
  ...
  void DsPPE::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/YOLOV5sPPEModel/config_infer_primary_yolov5s.txt");
    ...
    str(boost::format("models/YOLOV5sPPEModel/ppeModel.wts_b%d_gpu0_%s.engine") % num_sources % COMPUTE_MODE);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  ctx = g_option_context_new("DsPPE DeepStream App");
  group = g_option_group_new("DsPPE", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, DsPPE.entries);
  ...
  gint sources = DsPPE.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  int fail_safe = DsPPE.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, DsPPE.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      DsPPE.tiler_src_pad_buffer_probe, NULL, NULL);
  }
